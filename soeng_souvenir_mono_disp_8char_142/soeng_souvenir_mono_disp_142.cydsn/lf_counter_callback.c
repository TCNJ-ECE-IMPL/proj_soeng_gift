...
#define BANNER_TIME_IDX ( (int)(sizeof(BANNER_STR)-8-1) )   // -4 account for time chars, -1 to account for NULL
...
void lf_counter_callback(void)
{
    ...
    static uint8_t     str_pels[NUM_CHAR_TO_RENDER][8];
    static uint16_t    disp_cmds[8][NUM_MODULES_TOTAL];
    ...
    if (splashing_flag)
    {
        if (animation_count < ROAR_FRAMES)
        {
            ...
            get_pels_from_str( "  ROAR   ", 0, NUM_CHAR_TO_RENDER, str_pels );
            get_disp_cmds_frac_shift( str_pels, 0, disp_cmds, -(animation_count&1), START_MODULE_TEXT, NUM_MODULES_TEXT );
            ++animation_count;
        }
        ...
        // Then banner display
        {
            ...
            {
                // Get pixels for the next NUM_CHAR_TO_RENDER characters
                get_pels_from_str( banner_string, banner_shift_idx, NUM_CHAR_TO_RENDER, str_pels );
                get_disp_cmds_frac_shift( str_pels, frac_shift_idx, disp_cmds, 0, START_MODULE_TEXT, NUM_MODULES_TEXT );
        ...
        get_disp_cmds_frac_shift( str_pels, 0, disp_cmds, 0, START_MODULE_TIME, NUM_MODULES_TIME );
        // End time display
    ...
    if (clk_sec == 60)
    {
        clk_sec = 0;
        ++clk_min;
        both_btn = 1;               // triggers the restart of banner sequence
